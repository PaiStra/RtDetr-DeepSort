"""
当运行一次程序时，帧检测实时写入JSON文件中,包含世界坐标
"""
import cv2
import torch
...
def main():
    ...
    ip_camera_url = 'rtsp://admin:admin@172.18.192.226:8554/live'
    ...
    # 动态生成文件名
    run_counter = len(os.listdir('runs')) + 1
    json_filename = os.path.join('runs', f'coord{run_counter}.json')

    # 用于保存所有帧的数据
    all_frame_data = {"data": {"车": {}, "人": {}}}
    ...
    def display_frame():
        while True:
            try:
                ...
                # 将当前帧数据合并到所有帧数据中
                for category in ["车", "人"]:
                    for track_id, data in current_frame_data[category].items():
                        if track_id not in all_frame_data["data"][category]:
                            all_frame_data["data"][category][track_id] = []
                        all_frame_data["data"][category][track_id].extend(data)
                ...
                with open(json_filename, 'w', encoding='utf-8') as f:
                    json.dump(all_frame_data, f, ensure_ascii=False, indent=4)
